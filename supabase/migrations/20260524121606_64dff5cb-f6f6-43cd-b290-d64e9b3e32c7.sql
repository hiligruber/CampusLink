-- Pickup info on bookings
ALTER TABLE public.bookings
  ADD COLUMN IF NOT EXISTS pickup_location text,
  ADD COLUMN IF NOT EXISTS pickup_lat double precision,
  ADD COLUMN IF NOT EXISTS pickup_lng double precision;

-- Update ride_phase constraint to include 'picked_up'
ALTER TABLE public.rides DROP CONSTRAINT IF EXISTS rides_ride_phase_check;
ALTER TABLE public.rides
  ADD CONSTRAINT rides_ride_phase_check
  CHECK (ride_phase IN ('scheduled','en_route','picked_up','in_progress','completed'));

-- Extend phase-change notifier
...
BEGIN
  ...
  ELSIF NEW.ride_phase = 'picked_up' THEN
    v_title := 'הנהג סימן שאספו אותך ✅';
    v_body  := 'בקרוב יוצאים ליעד: ' || NEW.destination;
