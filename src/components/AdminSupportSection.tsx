...
const AdminSupportSection = () => {
  ...
  const { data: tickets = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      return (data ?? []).map((t) => ({ ...t, profile: profMap[t.user_id] as any }));
    },
  });
  ...
  const unblockUser = async (userId: string, ticketId: string) => {
    ...
    toast.success("המשתמש שוחרר ואושר");
  };
  ...
  return (
    <div className="bg-card rounded-2xl border border-border p-4 space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h2 className="font-bold">פניות תמיכה</h2>
      </div>
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-muted-foreground text-center py-4">אין פניות תואמות</p>
      ) : (
        <div className="space-y-2">
          {tickets.map((t: any) => {
            const isOpen = openId === t.id;
            const isBlocked = t.profile?.verification_status === "rejected";
            return (
              <div key={t.id} className="rounded-xl border border-border overflow-hidden">
                <button onClick={() => setOpenId(isOpen ? null : t.id)}
                  className="w-full text-right p-3 bg-secondary/30 hover:bg-secondary/50 transition">
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <p className="font-semibold text-sm truncate">{t.subject}</p>
                      <p className="text-[11px] text-muted-foreground mt-0.5">
                        {CAT[t.category]} · {t.profile?.full_name || "ללא שם"} · {t.profile?.email}
                      </p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                      <span className={`text-[10px] font-bold rounded-full px-2 py-0.5 ${STATUS_STYLE[t.status]}`}>
                        {STATUS[t.status]}
                      </span>
                ...
                {isOpen && (
                  <div className="p-3 space-y-3 bg-background">
                        ...
                        <div key={m.id} className={`flex ${m.is_admin ? "justify-end" : "justify-start"}`}>
                          <div className={`max-w-[85%] rounded-2xl px-3 py-2 text-sm ${
                            ...
                            <p className={`text-[10px] mt-1 ${m.is_admin ? "text-white/70" : "text-muted-foreground"}`}>
                              {m.is_admin ? "תמיכה" : "משתמש"} · {new Date(m.created_at).toLocaleString("he-IL")}
                            </p>
                          </div>
                        </div>
                      ))}
                    ...
                    <div className="flex gap-2 flex-wrap">
                      ...
                      {t.category === "appeal" && isBlocked && (
                        <Button size="sm" variant="outline" onClick={() => unblockUser(t.user_id, t.id)}>
                          שחרר חסימה ואשר משתמש
                        </Button>
                      )}
                    </div>
                  </div>
